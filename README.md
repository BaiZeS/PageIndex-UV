...
- **多文档联合检索**：Super-Tree Retrieval v3 四层架构（L0 四通道预过滤 → L1 LLM 选档 → L2 层级树导航 → L3 按相关性排序上下文提取）
- **层级树导航**：逐层四通道初筛 + 图谱加权(层级感知) + LLM 精挑，渐进披露
- **图谱加权**：L0 预计算实体距离(一次 BFS)，逐层查表(O(1))，关系越近权重越高
