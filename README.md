...
│   ├── page_index_liteparse.py # LiteParse 格式解析（docx/pptx/xlsx 等）
...
│   ├── reasoning.py           # 推理逻辑（LLM 调用、节点选择、答案生成）
│   ├── client.py              # PageIndexClient（单/多文档检索统一入口）
│   ├── utils.py               # 通用工具函数 (API 调用, Token 计数, 配置加载等)
...
除了 PDF 和 Markdown，系统还支持通过 LiteParse 处理多种文档格式：

*   **支持格式**: docx, doc, pptx, ppt, xlsx, xls, odt, ods, odp, png, jpg, jpeg, tiff, tif, bmp, gif, webp, html, htm, txt, csv, rtf
*   **处理方式**: 自动检测文件格式，调用相应的解析器提取文本和结构
*   **索引流程**: 与 PDF/Markdown 类似，生成结构化树索引并存储到 SQLite
...
| `/api/documents/{doc_id}` | DELETE | 删除指定文档 |
| `/api/search` | POST | RAG 检索接口 |
| `/api/config` | GET/POST | 获取/更新配置 |
| `/api/config/test` | POST | 测试模型连接 |

**认证**: 所有端点 (除 `/health` 和 `/static/*` 外) 需在请求头携带 `X-API-Key`。
